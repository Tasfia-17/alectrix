...
const OptionButton = React.memo(({ option, onClick, disabled, className }: { option: { text: string }, onClick: (option: string) => void, disabled: boolean, className: string }) => {
    return (
        <button
            onClick={() => onClick(option.text)}
            disabled={disabled}
            className={`p-3 rounded-xl shadow-md border text-md font-semibold transition-all duration-300 ${className} text-aurora-dark-text text-left`}
        >
            {option.text}
        </button>
    );
...
const MiniAdventureScreen: React.FC<MiniAdventureScreenProps> = ({ gameData, avatar, onBack }) => {
    ...
    const getOptionClass = (optionText: string) => {
        ...
        const isCorrectAnswer = optionText === currentStage.challenge.correctAnswer;
        const isSelectedOption = optionText === selectedOption;

        if (isCorrectAnswer) {
            return 'bg-green-300 border-green-400 scale-105';
        }
        if (isSelectedOption && optionText !== currentStage.challenge.correctAnswer) {
