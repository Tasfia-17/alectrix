...
const DraggableItem = React.memo(({ item, feedbackClass, onDragStart }: { item: Item, feedbackClass: string, onDragStart: (e: React.DragEvent<HTMLDivElement>, item: Item) => void }) => {
    return (
        <div 
            key={item.id}
            draggable 
            onDragStart={(e) => onDragStart(e, item)}
            className={`p-2 rounded-lg bg-white/70 shadow-sm cursor-grab active:cursor-grabbing border ${feedbackClass}`}
        >
            {item.text}
        </div>
    );
                                ...
                                <div className="flex flex-wrap gap-2 justify-center">
                                    {locations[category]?.map(item => (
                                        <DraggableItem
                                            ...
                                            feedbackClass={getFeedbackClass(item.id)}
                                        />
                                    ))}
                                </div>
                            </div>
                         ...
                         <div className="flex flex-wrap gap-2 justify-center">
                            {locations['unassigned']?.map(item => (
                                <DraggableItem
                                    ...
                                    item={item}
                                    onDragStart={handleDragStart}
                                    feedbackClass="border-transparent"
                                />
                            ))}
                         </div>
                    </div>
