...
const OptionButton = React.memo(({ option, onClick, disabled, className }: { option: StoryNode['options'][0], onClick: (option: StoryNode['options'][0]) => void, disabled: boolean, className: string }) => (
    <button 
...
const StoryBasedChallengeScreen: React.FC<StoryBasedChallengeScreenProps> = ({ gameData, onBack }) => {
    ...
    const handleOptionClick = useCallback((option: StoryNode['options'][0]) => {
